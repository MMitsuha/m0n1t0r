...
impl QZone {
    pub fn new(qq: Arc<QQ>, uin: i64, g_tk: String) -> Self {
        Self { qq, uin, g_tk }
    }
    ...
    pub fn special_cares_url(&self) -> String {
    ...
    pub fn friendships_rank_url(&self, to_self: bool) -> String {
        let r#do = match to_self {
            true => 2,
            false => 1,
        };
    ...
    async fn data_internal(&self, url: String) -> QQResult<String> {
        let response = self
            .qq
    ...
    pub async fn friends(&self) -> QQResult<HashMap<i64, Group>> {
        let response = serde_json::from_str::<FriendsResponse>(
            &self.data_internal(self.friends_url()).await?,
        )?;
    ...
    pub async fn special_cares(&self) -> QQResult<Vec<CaredFriend>> {
        let ret = serde_json::from_str::<SpecialCareResponse>(
            &self.data_internal(self.special_cares_url()).await?,
        )?;
    ...
    pub async fn friendships_rank(&self, to_me: bool) -> QQResult<Vec<FriendRank>> {
        let ret = serde_json::from_str::<FriendshipRankResponse>(
            &self.data_internal(self.friendships_rank_url(to_me)).await?,
        )?;
