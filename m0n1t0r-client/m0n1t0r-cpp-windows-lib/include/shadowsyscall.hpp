...
template <typename Ty>
concept chrono_duration = std::is_base_of_v<
    std::chrono::duration<typename Ty::rep, typename Ty::period>, Ty>;
template <typename Ty>
concept function_type =
    std::is_pointer_v<Ty> && std::is_function_v<std::remove_pointer_t<Ty>>;
...
template <typename ParamType, typename ArgType> struct arg_compatible {
  ...
  static constexpr bool is_pointer_compatible() {
    ...
    if constexpr (std::is_pointer_v<ArgDecayedT>) {
      ...
      if constexpr (allow_voidptr_for_ptr_param &&
                    std::is_same_v<ArgDecayedT, void *>)
        return true;
    ...
    if constexpr (allow_integrals_for_ptr_param &&
                  std::is_integral_v<ArgDecayedT>) {
      return true;
    }
  ...
  static constexpr bool value =
      std::is_pointer_v<ParamNoCVT>
          ? is_pointer_compatible()
          : std::is_convertible_v<ArgDecayedT, ParamNoCVT>;
};

template <typename F, typename... Args> struct args_compatible {
  using traits = function_traits<F>;
  static constexpr bool count_ok = (sizeof...(Args) == traits::arity);

  template <std::size_t... I>
  static constexpr bool check(std::index_sequence<I...>) {
    return (arg_compatible<typename traits::template argument_type<I>,
                           Args>::value &&
            ...);
  }

  static constexpr bool value =
      count_ok && check(std::make_index_sequence<traits::arity>{});
};
...
[[maybe_unused]] constexpr auto bitness =
...
public:
  ...
  [[nodiscard]] constexpr PointerTy
  ptr(std::ptrdiff_t offset = 0) const noexcept {
  ...
  [[nodiscard]] constexpr Ty offset(std::ptrdiff_t offset = 0) const noexcept {
    if constexpr (std::is_pointer_v<Ty>)
      return m_address == 0u ? nullptr
                             : reinterpret_cast<Ty>(m_address + offset);
    else
      return m_address == 0u ? static_cast<Ty>(*this) : Ty{m_address + offset};
  }

  template <typename Ty> [[nodiscard]] constexpr Ty as() const noexcept {
    if constexpr (std::is_pointer_v<Ty>)
  ...
  [[nodiscard]] bool is_in_range(const address_t &start,
                                 const address_t &end) const noexcept {
  ...
  [[nodiscard]] constexpr address_t
  operator+(const address_t &rhs) const noexcept {
  ...
  [[nodiscard]] constexpr address_t
  operator&(const address_t &other) const noexcept {
  ...
  [[nodiscard]] constexpr address_t
  operator<<(std::size_t shift) const noexcept {
...
inline auto split_forwarder_string(std::string_view view,
...
public:
  ...
  [[nodiscard]] auto as() const
      noexcept(std::is_nothrow_constructible_v<Ty, pointer_t>) {
    return Ty{m_buffer};
  }

  [[nodiscard]] auto to_path(std::filesystem::path::format fmt =
                                 std::filesystem::path::auto_format) const {
...
#pragma warning(pop)
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  const unicode_string &unicode_str) {
    return os << unicode_str.string();
  }

private:
  [[nodiscard]] bool
  contains_non_ascii(const std::wstring_view str) const noexcept {
...
template <std::ranges::view StrTy,
          typename CharTy = std::ranges::range_value_t<StrTy>>
...
struct optional_header_x86_t {
  ...
  inline bool has_directory(const data_directory_t *dir) const {
    return &data_directories.entries[num_data_directories] < dir &&
           dir->present();
  }
...
struct nt_headers_t {
  ...
  inline section_header_t *get_sections() {
    return (section_header_t *)((uint8_t *)&optional_header +
                                file_header.size_optional_header);
  }
...
struct image_t {
  ...
  inline data_directory_t *get_directory(directory_id id) {
    ...
    if (nt_hdrs->optional_header.num_data_directories <= id)
      return nullptr;
    data_directory_t *dir =
        &nt_hdrs->optional_header.data_directories.entries[id];
  ...
  inline const T *rva_to_ptr(uint32_t rva, size_t length = 1) const {
    return const_cast<image_t *>(this)->template rva_to_ptr<const T>(rva,
                                                                     length);
  }
...
inline auto image_from_base(address_t base) { return base.ptr<image_t>(); }
...
constexpr T *containing_record(FieldT *address, FieldT T::*field) {
  auto offset =
      reinterpret_cast<std::uintptr_t>(&(reinterpret_cast<T *>(0)->*field));
...
#endif
template <typename Ty> struct type_hash {
...
template <typename Ret, typename... Args> class stack_function;

template <typename Ret, typename... Args> class stack_function<Ret(Args...)> {
public:
  ...
  stack_function(F &&func) {
    static_assert(sizeof(DecayedF) <= sizeof(m_storage),
                  "Function object too large");
...
template <std::integral Ty = std::uint64_t> struct hasher_opts {
...
class basic_hash {
  constexpr static ValTy FNV_prime =
      (sizeof(ValTy) == 4) ? 16777619u : 1099511628211ull;
...
public:
  ...
  [[nodiscard]] constexpr ValTy get() const { return m_value; }
...
private:
  ...
  [[nodiscard]] constexpr ValTy
  fnv1a_append_bytes(ValTy value, const CharTy byte) const noexcept {
...
public:
  cpu_info() noexcept { parse_cpu_fields(); }
...
private:
    ...
    void parse_cache_info() noexcept {
      ...
      for (int i = 0;; ++i) {
        ...
        std::int32_t cache_size =
            ...
            ((cpu_info[2] & 0x3FF) +
             1) *              // Associativity (ways of set associativity)
  ...
  std::string extract_cpu_vendor() noexcept {
    ...
    std::array<int, 3> vendor_ids = {
        m_standard_data[0][1], m_standard_data[0][3], m_standard_data[0][2]};
    ...
    // Intel || Intel (rare)
    if (hashed_str == hash64_t("GenuineIntel") ||
        hashed_str == hash64_t("GenuineIotel")) {
      m_is_intel = true;
    }
    // AMD || Early samples of AMD K5 processor
    else if (hashed_str == hash64_t("AuthenticAMD") ||
             hashed_str == hash64_t("AMD ISBETTER")) {
      m_is_amd = true;
    }
  ...
  std::string extract_cpu_brand() const noexcept {
    ...
    memcpy(vendor_bytes.data(), m_extended_data[2].data(),
           sizeof(m_extended_data[2]));
    memcpy(vendor_bytes.data() + 16, m_extended_data[3].data(),
           sizeof(m_extended_data[3]));
    memcpy(vendor_bytes.data() + 32, m_extended_data[4].data(),
           sizeof(m_extended_data[4]));
...
template <std::integral Ty = std::size_t> class memory_checksum {
...
private:
  void process_block(const std::span<const uint8_t> data, std::size_t pos,
    ...
    auto block =
        _mm_loadu_si128(reinterpret_cast<const vector128_t *>(&data[pos]));
  ...
  Ty append_tail(const std::span<const uint8_t> data, std::size_t pos) const {
    return std::reduce(data.begin() + pos, data.end(), Ty{},
                       [](Ty acc, char byte) {
...
class api_set_contract {
  ...
  template <std::integral Ty>
  Ty convert_version_symbols_to_integral(
      std::wstring_view symbols) const noexcept {
    using namespace std::ranges;

    auto ascii_view =
        symbols | views::filter([](wchar_t c) { return c < 128; }) |
...
public:
  ...
  [[nodiscard]] auto version() const noexcept {
    ...
    // Contract name should have the following format:
    // "-l<major>-<minor>-<micro>"
    ...
    if (!major_str.empty() &&
        (major_str.front() == L'l' || major_str.front() == L'L')) {
...
private:
  [[nodiscard]] bool
  compare_wide_with_narrow(std::wstring_view wide,
...
public:
  api_set_host_range(const win::api_set_value_entry *entries, uint32_t count,
                     address_t base)
  ...
  public:
    ...
    iterator(const win::api_set_value_entry *entries, uint32_t count,
             address_t base, uint32_t index = 0)
  ...
  private:
    void update_value() {
      ...
      auto value_string_length =
          static_cast<uint16_t>(entry.value_length / sizeof(wchar_t));
      ...
      if (entry.name_length != 0) {
        ...
        auto alias_string_length =
            static_cast<uint16_t>(entry.name_length / sizeof(wchar_t));
...
public:
  api_set_entry()
      : m_value_entries(nullptr), m_value_count(0), m_base(0), m_sealed(false) {
  }

  api_set_entry(std::wstring_view contract_name, bool sealed,
                const win::api_set_value_entry *entries, uint32_t count,
                address_t base)
      : m_contract(contract_name), m_sealed(sealed), m_value_entries(entries),
        m_value_count(count), m_base(base) {}
...
public:
  api_set_map_view() {
    ...
    m_api_set_map =
        reinterpret_cast<const win::api_set_namespace *>(peb->reserved9[0]);
    if (!m_api_set_map) {
      m_count = 0;
    ...
    m_first_entry = m_base.offset<const win::api_set_namespace_entry *>(
        m_api_set_map->entry_offset);
  }
  ...
  private:
    void update_value() {
      ...
      auto name_string_ptr =
          m_enumerator->m_base.offset<wchar_t *>(ns_entry->name_offset);
      auto name_string_length =
          static_cast<uint16_t>(ns_entry->name_length / sizeof(wchar_t));
      std::wstring_view contract_name{name_string_ptr, name_string_length};

      auto value_entry =
          m_enumerator->m_base.offset<const win::api_set_value_entry *>(
              ns_entry->value_offset);

      m_current = api_set_entry(contract_name, sealed, value_entry,
                                ns_entry->value_count, m_enumerator->m_base);
    }
...
public:
  ...
  [[nodiscard]] auto name(std::size_t index) const noexcept {
    ...
    const auto rva_names_span =
        rva_names_ptr.span<const uint32_t>(m_export_dir->num_names);
    const auto export_name_ptr =
        m_module_base.offset<const char *>(rva_names_span[index]);
  ...
  [[nodiscard]] auto
  is_export_forwarded(address_t export_address) const noexcept {
    ...
    const auto export_table_end =
        export_table_start.offset(export_data_dir.size);

    return (export_address >= export_table_start) &&
           (export_address < export_table_end);
  }
  ...
  private:
    ...
    void reset_value() noexcept { m_value = value_type{}; }
  ...
  // \return Iterator pointing to [name, address] if export is found, or .end()
  ...
  [[nodiscard]] iterator find(hash64_t export_name) const noexcept {
    ...
    auto it = std::ranges::find_if(
        *this, [export_name](const win::export_t &data) -> bool {
  ...
  // \return Iterator pointing to [name, address] if export is found, .end() if
  // export is not found.
  [[nodiscard]] iterator
  find_if(std::predicate<iterator::value_type> auto predicate) const {
    return std::ranges::find_if(*this, predicate);
  }

private:
  win::export_directory_t *
  get_export_directory(address_t base_address) const noexcept {
...
public:
  ...
  dynamic_link_library(hash64_t module_name)
      : m_data(find(module_name).loader_table_entry()) {}
  dynamic_link_library(win::loader_table_entry *module_data)
      : m_data(module_data) {}
  ...
  operator=(const dynamic_link_library &instance) = default;
  ...
  [[nodiscard]] auto section_checksum(hash32_t section_name = ".text") const {
    ...
    const auto section_content =
        std::span{module_base.ptr<uint8_t>(section->virtual_address),
  ...
  [[nodiscard]] bool
  operator==(const dynamic_link_library &other) const noexcept {
  ...
  [[nodiscard]] bool operator==(hash64_t module_name_hash) const noexcept {
    ...
    // Verify both hashes
    return full_name_hash == module_name_hash ||
           trimmed_name_hash == module_name_hash;
  }

  [[nodiscard]] explicit operator bool() const noexcept { return present(); }

  friend std::ostream &operator<<(std::ostream &os,
                                  const dynamic_link_library &dll) {
    return os << dll.name();
  }
...
public:
  ...
  private:
    void on_update() const noexcept {
      auto table_entry = win::containing_record(
          m_entry, &win::loader_table_entry::in_load_order_links);
...
public:
  ...
  explicit exported_symbol(hash64_t export_name,
                           hash64_t module_hash = 0) noexcept
...
private:
  ...
  export_with_location
  find_export_by_ordinal(hash64_t module_hash,
    ...
    for (const auto &module : process_modules) {
      ...
      const auto predicate_by_name =
          [ordinal](const win::export_t &exp) -> bool {
        return exp.ordinal == ordinal;
      };

      if (auto export_it = exports.find_if(predicate_by_name);
          export_it != exports.end()) {
  ...
  export_with_location
  find_export_address(hash64_t export_name,
    ...
    for (const auto &module : process_modules) {
      ...
      const auto predicate_by_name =
          [export_name](const win::export_t &data) -> bool {
        return export_name == hash64_t{}(data.name);
      };
  ...
  export_with_location handle_forwarded_export(address_t address) const {
    ...
    if (!token.empty() && token.front() == '#') {
      ...
      auto conversion_result = std::from_chars(
          ordinal_str.data(), ordinal_str.data() + ordinal_str.size(),
          real_ordinal);

      if (conversion_result.ec == std::errc{})
        real_export =
            find_export_by_ordinal(hash64_t{}(module_name), real_ordinal);
    } else {
      real_export =
          find_export_address(hash64_t{}(token), hash64_t{}(module_name));
    }
...
public:
  operating_system(std::uint32_t major, std::uint32_t minor,
                   std::uint32_t build_num) noexcept
      : m_major_version(major), m_minor_version(minor),
        m_build_number(build_num) {}
  ...
  [[nodiscard]] auto major_version() const noexcept { return m_major_version; }
  [[nodiscard]] auto minor_version() const noexcept { return m_minor_version; }
  [[nodiscard]] auto build_number() const noexcept { return m_build_number; }

  [[nodiscard]] auto formatted() const noexcept {
    return std::format("Windows {}.{} (Build {})", m_major_version,
                       m_minor_version, m_build_number);
  }
...
public:
  ...
  [[nodiscard]] auto format_european() const {
    auto [year, month, day, hours, minutes, _] =
        break_down_unix_time(m_unix_seconds);
    return std::format("{:02}.{:02}.{} {:02}:{:02}", day, month, year, hours,
                       minutes);
  }
  ...
  [[nodiscard]] auto format_american() const {
    auto [year, month, day, hours, minutes, _] =
        break_down_unix_time(m_unix_seconds);
    return std::format("{:02}/{:02}/{} {:02}:{:02}", month, day, year, hours,
                       minutes);
  }

  // \return ISO 8601 format: "yyyy-mm-ddThh:mm:ss"
  [[nodiscard]] auto format_iso8601() const {
    auto [year, month, day, hours, minutes, seconds] =
        break_down_unix_time(m_unix_seconds);
    return std::format("{}-{:02}-{:02}T{:02}:{:02}:{:02}", year, month, day,
  ...
  timestamp break_down_unix_time(std::uint64_t unix_timestamp) const {
    ...
    timestamp stamp{
        ...
        .hours = static_cast<int32_t>(
            std::chrono::duration_cast<std::chrono::hours>(time_since_midnight)
                .count()),
...
class zoned_time {
public:
  constexpr zoned_time(std::uint64_t unix_timestamp,
                       std::int64_t timezone_offset) noexcept
...
public:
  ...
  using hundred_ns_interval =
      std::chrono::duration<int64_t, std::ratio<1, 10000000>>;

public:
  constexpr shared_data()
      : m_data(memory_location.ptr<win::kernel_user_shared_data>()) {}

  [[nodiscard]] auto *get() const noexcept { return m_data; }
  ...
  [[nodiscard]] auto timezone_offset() const
      noexcept(std::is_nothrow_constructible_v<Ty>) {
  ...
  [[nodiscard]] auto windows_epoch_timestamp() const {
    ...
    const auto windows_time_100ns =
        static_cast<uint64_t>(system_time.high1_time) << 32 |
        system_time.low_part;
  ...
  [[nodiscard]] auto unix_epoch_timestamp() const {
    ...
    const auto windows_time_seconds =
        std::chrono::duration_cast<std::chrono::seconds>(
            hundred_ns_interval(windows_time_100ns));
    return zoned_time{static_cast<uint64_t>(
                          (windows_time_seconds - epoch_difference).count()),
...
private:
  std::int64_t parse_time_zone_bias() const {
    ...
    const auto bias_100ns =
        (static_cast<std::int64_t>(bias.high1_time) << 32) | bias.low_part;
    ...
    const auto bias_seconds = std::chrono::duration_cast<std::chrono::seconds>(
        hundred_ns_interval(bias_100ns));
...
template <typename ValueTy, typename KeyTy> class memory_cache {
public:
...
inline memory_cache<detail::exported_symbol, hash64_t::underlying_t>
    address_cache;

#endif

template <typename Ty> auto convert_nulls_to_nullptrs(Ty arg) {
  ...
  using unsigned_integral_type =
      std::conditional_t<is_unsigned_integral, std::uintptr_t, Ty>;
  using tag_type = std::conditional_t<is_signed_integral, std::intptr_t,
                                      unsigned_integral_type>;

  return static_cast<tag_type>(arg);
}

template <auto Fn> consteval std::string_view extract_function_name() {
  ...
  // "... extract_function_name() [with auto Fn = MessageBoxA; std::string_view
  ...
  // "... extract_function_name<int __cdecl A::B::FunctionNameA(int,
  // int*)>(void)"
  constexpr std::string_view sig{__FUNCSIG__};
  constexpr std::string_view marker{"extract_function_name<"};

  constexpr std::size_t after =
      sig.find(marker) + marker.size(); // "... A::B::FunctionNameA("
  ...
  constexpr std::size_t name_begin =
      (sep == std::string_view::npos) ? 0 : sep + 1;

  // "A::B::FunctionNameA" or just "FunctionNameA"
  constexpr auto ident =
      left_part.substr(name_begin, name_end - name_begin + 1);
...
struct hashpair {
  consteval hashpair(hash64_t first_, hash64_t second_)
...
inline auto dll(hash64_t name) { return detail::dynamic_link_library{name}; }

inline auto base_module() { return *(detail::module_view{}.begin()); }
...
inline auto exported_symbol(detail::use_ordinal_t, hash64_t module,
                            std::uint32_t ordinal) {
  return detail::exported_symbol(module, ordinal);
}

inline auto dlls() { return detail::module_view{}.skip_module(); }

inline auto exported_symbols(hash64_t module_name) {
  return detail::export_view{dll(module_name).base_address()};
}

inline auto shared_data() { return detail::shared_data{}; }

inline auto api_set_map_view() { return detail::api_set_map_view{}; }
...
public:
  ...
  template <typename PtrTy> void deallocate(PtrTy p, std::size_t n) noexcept {
    std::size_t size = n * sizeof(Ty);
...
private:
  ...
  void *virtual_alloc(void *address, std::uint64_t allocation_size,
                      std::uint32_t allocation_t, std::uint32_t protect) const {
...
#else
    auto allocation_procedure =
        detail::cached_alloc_proc.load(std::memory_order_acquire);
    if (!allocation_procedure) {
      auto addr =
          exported_symbol(hash64_t{"NtAllocateVirtualMemory"}, "ntdll.dll")
              .address();
    ...
    auto result = allocation_procedure.execute<NTSTATUS>(
        current_process, &base_address, 0ull, &region_size,
        allocation_t & 0xFFFFFFC0, protect);
    return result >= 0 ? base_address : nullptr;
  }

  bool virtual_free(void *address, std::uint64_t allocation_size,
...
#else
    auto free_procedure =
        detail::cached_free_proc.load(std::memory_order_acquire);
    if (!free_procedure) {
      auto addr = exported_symbol(hash64_t{"NtFreeVirtualMemory"}, "ntdll.dll")
                      .address();
    ...
    if (((flags & 0xFFFF3FFC) != 0 || (flags & 0x8003) == 0x8000) &&
        allocation_size) {
      result = -0x3FFFFFF3;
    }
...
template <typename Ty>
using rw_allocator =
    nt_memory_allocator<Ty, mem::commit_reserve, mem::page::read_write>;
template <typename Ty>
using rx_allocator =
    nt_memory_allocator<Ty, mem::commit_reserve, mem::page::execute_read>;
template <typename Ty>
using rwx_allocator =
    nt_memory_allocator<Ty, mem::commit_reserve, mem::page::execute_read_write>;
template <typename Ty>
using huge_allocator =
    nt_memory_allocator<Ty, mem::large_commit, mem::page::read_write>;

template <std::uint32_t shell_size> class shellcode {
public:
  template <class... Args>
    requires((std::is_convertible_v<Args, std::uint8_t> && ...) &&
             shell_size != 0)
...
template <concepts::fundamental Ty = long,
          bool IsTypeNtStatus = concepts::is_type_ntstatus<Ty>>
  requires(shadow::is_x64)
class syscaller {
public:
  // Parser needs to return std::optional<uint32_t> and accept (syscaller&,
  // address_t)
  using ssn_parser_t =
      detail::stack_function<std::optional<uint32_t>(syscaller &, address_t)>;

public:
  constexpr syscaller(hash64_t syscall_name) noexcept(!SHADOW_HAS_EXCEPTIONS)
      : m_name_hash(syscall_name), m_service_number(0),
        m_last_error(std::nullopt),
        m_ssn_parser(
            [this]([[maybe_unused]] syscaller &instance, address_t address) {
...
private:
  ...
  std::uint32_t default_ssn_parser(address_t export_address) {
    ...
    for (auto i = 0; i < 24; ++i) {
      if (address[i] == 0x4c && address[i + 1] == 0x8b &&
          address[i + 2] == 0xd1 && address[i + 3] == 0xb8 &&
...
public:
  ...
  [[nodiscard]] auto exported_symbol() const noexcept { return m_export; }
...
private:
  detail::exported_symbol get_export(hash64_t export_name,
                                     hash64_t module_name) {
#ifndef SHADOWSYSCALLS_DISABLE_CACHING
...
inline auto shadowcall(hashpair export_and_module_names, Args &&...args) {
  ...
  const auto &[export_name, module_name] = export_and_module_names;
  if constexpr (Traits::is_void) {
    importer<return_type>{export_name,
                          module_name}(std::forward<Args>(args)...);
  } else {
    return importer<return_type>{export_name,
                                 module_name}(std::forward<Args>(args)...);
  }
}
...
inline auto shadowcall(Args &&...args) {
  ...
  if constexpr (Traits::is_void) {
    ...
    return importer<return_type>{func_hash,
...
template <>
struct formatter<shadow::address_t>
    : shadow::detail::type_format<shadow::address_t> {};
template <>
struct formatter<shadow::hash32_t>
    : shadow::detail::type_format<shadow::hash32_t> {};
template <>
struct formatter<shadow::hash64_t>
    : shadow::detail::type_format<shadow::hash64_t> {};
} // namespace std
