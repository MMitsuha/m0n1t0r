...
std::wstring utf8_to_wstring(rust::String string) {
  ...
  auto wide_len = shadowcall<int>("MultiByteToWideChar", CP_UTF8, 0, ptr,
                                  string.size(), nullptr, 0);
  if (wide_len <= 0) {
    throw AppError("failed to get wide len");
  }
  std::wstring wide_string(wide_len, 0);
  if (shadowcall<int>("MultiByteToWideChar", CP_UTF8, 0, ptr, string.size(),
                      wide_string.data(), wide_len) <= 0) {
...
std::wstring acp_to_wstring(rust::Vec<uint8_t> string) {
  ...
  auto wide_len = shadowcall<int>("MultiByteToWideChar", CP_ACP, 0, ptr,
                                  string.size(), nullptr, 0);
  if (wide_len <= 0) {
    throw AppError("failed to get wide len");
  }
  std::wstring wide_string(wide_len, 0);
  if (shadowcall<int>("MultiByteToWideChar", CP_ACP, 0, ptr, string.size(),
                      wide_string.data(), wide_len) <= 0) {
...
rust::String wstring_to_utf8(std::wstring &wstring) {
  ...
  auto utf8_len = shadowcall<int>("WideCharToMultiByte", CP_UTF8, 0, ptr,
                                  wstring.size(), nullptr, 0, nullptr, nullptr);
  if (utf8_len <= 0) {
    throw AppError("failed to get utf8 len");
  }
  rust::String utf8_string("", utf8_len);
  if (shadowcall<int>("WideCharToMultiByte", CP_UTF8, 0, ptr, wstring.size(),
...
uint32_t acp() { return shadowcall<UINT>("GetACP"); }
